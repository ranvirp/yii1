...
class RevenueVillage extends CActiveRecord
{
	...
	*/
	...
	public static function listAllByOfficer()
	{
			$lang=Yii::app()->language;
		$name='name_'.$lang;
		$location=Designation::getLocationByUser();
		if ($location['locationType']=='district')
		{
			$sql ="create * from revenue_village where tehsil_code in (select code from tehsil where district_code=:district_code)";
			$command = Yii::app()->db->createCommand($sql);
            $command->bindValue(":district_code", $location['location_code'], PDO::PARAM_STR);
            
		}
		else if ($location['locationType']=='tehsil')
		{
			$sql ="create * from revenue_village where tehsil_code=:tehsil_code";
			$command = Yii::app()->db->createCommand($sql);
            $command->bindValue(":tehsil_code", $location['location_code'], PDO::PARAM_STR);
            
		}
		else if ($location['locationType']=='block')
		{
			$sql ="create * from revenue_village where panchayat_code in (select code from panchayat where block_code=:block_code)";
			$command = Yii::app()->db->createCommand($sql);
            $command->bindValue(":block_code", $location['location_code'], PDO::PARAM_STR);
            
		}
		else 
		{
			$sql ="create * from revenue_village where 0";
			$command = Yii::app()->db->createCommand($sql);
